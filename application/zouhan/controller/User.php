...
class User extends Base
{
    ...
    public function userInfo()
    {
        if (request()->isPost()) {
            $data = input('post.');
            db('owner')->strict(false)->update($data);
            return $this->success('用户信息更新成功！');

        } else {
            $user_id=session('user_id');
            $data = db('owner')->where('owner_id=' . $user_id)->find();
    ...
    public function modiPwd(){
        $data=input("post.");
        $user = db('owner')->where('owner_id', session("user_id"))->find();
        if($user['owner_password'] != md5($data['oldPsw'])){
            return $this->error('原密码错误!');
        }else{
            $data['owner_id']=(string)(session("user_id"));
            $data['owner_password']=md5($data['newPsw']);
            ...
            if (db('owner')->strict(false)->update($data)!==false){  //
