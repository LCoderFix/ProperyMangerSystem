    ...
    public function index(){
        if (request()->isAjax()) {
           ...
           // $where['order.status']=0;
            ...
            $list = db('order')
                ->join('owner o','o.owner_id=order.owner_id')
                ->join('park p','p.id=order.park_id')
                ->field('order.*,p.location,o.owner_name,o.owner_village')
                ->where($where)
                ->order('order.id')
                ->paginate($limit)  //分页
                ->toArray(); //转换为数组
    ...
    public function orderDel()
    {
       ...
       // var_dump($data['data']['status']);
        if($data['data']['status']=='1'){
            ...
            db('order')->where('id',$data['data']['id'])->update(['status'=>1]);
